...
fn main() {
    eprintln!("tcp_auto_slot_scope_probe requires Linux");
    std::process::exit(2);
}
...
mod linux {
    ...
    #[derive(Debug, Parser)]
    #[command(about = "Mixed-lifecycle auto splice slot-scope probe")]
    struct Args {
        #[arg(long, value_enum)]
        backend: Backend,
        ...
        #[arg(long, default_value_t = 8)]
        auto_slot_limit: usize,

        #[arg(long, default_value_t = 8)]
        slow_prelude_connections: usize,
    ...
    struct Summary {
        ...
        splice_hits_median: f64,
        copy_fallbacks_median: f64,
    }
    ...
    pub(super) fn run() -> Result<()> {
        ...
        let mut splice_hits = Vec::with_capacity(args.runs);
        let mut copy_fallbacks = Vec::with_capacity(args.runs);
        for run_index in 0..args.runs {
            ...
            splice_hits.push(record.splice_hits as f64);
            copy_fallbacks.push(record.copy_fallbacks as f64);
            println!("{}", serde_json::to_string(&record)?);
        }
        ...
        println!(
            ...
            serde_json::to_string(&Summary {
                ...
                splice_hits_median: round(median(&splice_hits)),
                copy_fallbacks_median: round(median(&copy_fallbacks)),
            })?
        );
        Ok(())
    }
    ...
    fn validate_args(args: &Args) -> Result<()> {
        ...
        if matches!(
            args.backend,
            Backend::AutoBeforePrelude | Backend::AutoAfterPrelude
        ) && (args.auto_slot_limit == 0
            || args.slow_prelude_connections >= args.connections)
        {
            bail!(
                "auto modes require a positive slot limit and at least one raw-ready connection"
            );
        }
        Ok(())
    }
    ...
    async fn run_once(
        ...
        for (flow_index, flow) in flows.into_iter().enumerate() {
            let config = FlowRunConfig {
                ...
                splice_pipe_size: args.splice_pipe_size,
                prefix_bytes: args.prefix_bytes,
                expected: args.bytes_per_connection,
                verify: args.verify,
                auto_slot_limit: args.auto_slot_limit,
                slow_prelude_connections: args.slow_prelude_connections,
        ...
        for task in tasks {
            ...
            {
                EffectivePath::Copy => copy_fallbacks += 1,
                EffectivePath::Splice => splice_hits += 1,
                EffectivePath::Other => {}
            }
        }
    ...
    async fn run_flow(
        ...
        let relay = async {
            match backend {
                ...
                Backend::AutoBeforePrelude | Backend::AutoAfterPrelude => {
                    let slow = flow_index < slow_prelude_connections;
                    let pre_reserved = backend == Backend::AutoBeforePrelude && slow;
                    if slow {
                    ...
                    if let Some(_slot) = slot {
                        ...
                        if transferred != expected as u64 {
                            return Err(io::Error::other(format!(
                                "auto splice count {transferred}, expected {expected}"
                            )));
                        }
                        Ok(EffectivePath::Splice)
                    } else {
                        ...
                        if left_to_right != expected as u64 || right_to_left != 0 {
                            return Err(io::Error::other(format!(
                                "auto copy counts {left_to_right}/{right_to_left}, expected {expected}/0"
                            )));
                        }
                        Ok(EffectivePath::Copy)
                    }
        ...
        let (path, ()) = tokio::try_join!(relay, sink)?;
        Ok(path)
    }
    ...
    mod tests {
        ...
        #[test]
        fn slot_reservation_enforces_limit_and_releases() {
            let active = Arc::new(AtomicUsize::new(0));
            let first = try_reserve_slot(&active, 1).expect("first slot");
            assert!(try_reserve_slot(&active, 1).is_none());
            assert_eq!(active.load(Ordering::Relaxed), 1);
            drop(first);
            assert_eq!(active.load(Ordering::Relaxed), 0);
            assert!(try_reserve_slot(&active, 1).is_some());
        }
    }
}
