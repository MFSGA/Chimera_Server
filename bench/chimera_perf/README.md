...
`--destination-drain-mode until-would-block` is an unpaced benchmark-only model of reusing one Tokio destination writable guard for repeated `pipe → socket` splices until the pipe empties or `try_io` observes `WouldBlock`, instead of reacquiring `destination.writable().await` after every successful partial splice. It also reports `destination_ready_acquisitions` so the saved reactor-facing work is measured directly. The September 2026 host does not show enough headroom to justify this in production. In an interleaved c64/64 MiB-per-flow run the drain mode reduced median destination readiness acquisitions only from about 558 to 537 per connection (~3.8%); throughput was effectively noise-limited and CPU/GiB did not improve consistently. A lower-noise c1/1 GiB run measured 17.998 vs 18.411 Gbit/s and 0.2969 vs 0.2937 CPU seconds/GiB for single vs drain, but both throughput CVs were still about 7-8% and readiness acquisitions changed by only ~0.2%. A focused c16 `strace -f -c` likewise moved tracked `splice + epoll + pipe/fcntl` calls only from 18,671 to 18,519 (-0.8%), with `splice` calls 17,909→17,792 and errors 705→628. Keep the production `SpliceDirection` state machine unchanged: the extra guard reuse removes too little reactor work to support a data-path complexity increase on the current evidence.
