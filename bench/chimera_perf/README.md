...
`--sample-writable-wake-every N` attributes the destination wake itself. After every Nth destination `WouldBlock`, it samples `SIOCOUTQNSD` and `TCP_INFO` immediately when `AsyncFd::writable().await` completes and before the next splice attempt. The run report includes the fraction of samples at or below the currently published `TCP_NOTSENT_LOWAT` plus mean unsent, unacked, and sender-cwnd bytes. The extra ioctl/getsockopt calls make this an attribution mode, not a CPU benchmark. A 31.5 ms RTT `32→25 MiB/s` run sampled 1,462 wakes across three measured c8 runs and an 82 ms RTT c4 run sampled 614 more; **100%** of wakes in both groups arrived with unsent bytes already at or below the active low-water. This closes the current readiness-bookkeeping hypothesis: the post-EAGAIN writable wake is a real kernel send-queue threshold event, while the high-RTT transition limit increasingly sits in already-unacked data. See `TCP_WRITABLE_WAKE_ATTRIBUTION.md` for the full setup and numbers.

`--sample-rate-decrease-flight` samples that higher-level boundary at **every** pacing decrease. Before publishing the new `SO_MAX_PACING_RATE`, it records `SIOCOUTQNSD`, `TCP_INFO` unacked/cwnd state, and computes the new-rate one-RTT BDP. Replaying the staged live Brutal sequence `13.11→13.76→14.45→15.26→14.50→13.77 MB/s` with a 32 ms queue-time low-water found that the two decreases arrived with `(unsent + unacked)` already at a median **1.866× new-rate BDP** around 34 ms RTT; unacked alone was **1.014× BDP**. Around 85 ms RTT, total admitted state was still **1.411× BDP** and unacked alone was **1.090× BDP**. This makes the high-RTT limitation explicit: by the time the current Brutal-derived decrease is published, at least one RTT of old-rate data is already in flight, so post-publication low-water/readiness tweaks cannot remove the dominant stale bytes. The mode adds one ioctl and one `TCP_INFO` getsockopt per decrease and is for attribution only. See `TCP_RATE_DECREASE_FLIGHT.md`.
...
A source-readiness follow-up tests a different Tokio overhead hypothesis. The production splice loop currently acquires `AsyncFd::readable()` before every `socket -> pipe` splice even though earlier attribution found source `WouldBlock` events to be almost absent. `--source-readiness-mode try-first` first calls readiness-aware `AsyncFd::try_io(READABLE)` and falls back to `readable().await` only when Tokio has no cached readable state or the syscall would block. It does not bypass Tokio's readiness bookkeeping or add a new syscall. At c64, 16 MiB/s/flow, 32 ms `TCP_NOTSENT_LOWAT`, and 128 KiB pipes, the candidate reduced source readiness acquisitions from about **130.1 to 0.73 per connection**, with about **129.3 direct cached-ready successes**. Eight 16 MiB/flow paired runs initially showed a 0.973x median CPU/GiB ratio (6/8 favorable), but a stricter 32 MiB/flow rerun reversed the apparent win: source acquisitions still fell from about **258.1 to 0.76 per connection**, yet candidate/baseline CPU/GiB had a **1.026x median** with only **3/10** favorable pairs. Throughput and relay max/median completion ratios stayed effectively flat (about 1.000x). A c16 `strace -f -c` also kept tracked kernel work essentially unchanged: guarded vs try-first measured 5728 vs 5724 `splice` calls and 4580 vs 4521 `epoll_wait` calls. Therefore reducing cached-ready guard acquisition is a real mechanism effect but not a stable CPU optimization on this workload; keep production `SpliceDirection` unchanged and retain this mode only as a regression/profiling asset.
...
`BRUTAL_RECIP_BENCH_MODE=baseline|reciprocal` isolates the loss-active controller arithmetic. The baseline computes `ack / total` and then divides the congestion window by that ACK rate; the reciprocal form computes `total / ack` once and multiplies the window by the result. Twelve alternating CPU-0-pinned pairs at 10% modeled loss measured medians of **10.280 ns/ACK vs 8.208 ns/ACK**, with a **0.786x** paired ratio median and **12/12** favorable pairs. Production therefore stores the reciprocal ACK rate internally, removing one floating-point division from each loss-active ACK without changing the rolling window or congestion formula. See `TCP_BRUTAL_RECIPROCAL_ACK_RATE.md`.

`BRUTAL_CLAMP_BENCH_MODE=baseline|integer` tests a tempting follow-up for severe loss: detect the 0.8 minimum ACK-rate clamp with the exact integer relation `ack < 4 * loss` before performing `total / ack`. The benchmark keeps the clamp active at a modeled 75% ACK rate and runs 100 million ACK events per process. After three warmup pairs, ten alternating CPU-0-pinned measured pairs produced **6.777 ns/ACK** baseline versus **7.194 ns/ACK** integer-threshold medians; the candidate/baseline ratio median was **1.060x**, and **0/10** pairs favored the candidate. Both variants ended at the same 0.8 ACK rate and modeled window. Keep the integer threshold as a rejected regression probe: the extra branch/multiply costs more than allowing the floating-point division and clamp on this CPU. See `TCP_BRUTAL_CLAMP_THRESHOLD.md`.
...
A follow-up component attribution keeps the same 10 kHz timestamp progression and adds a small deterministic RTT movement on every modeled ACK, matching the live observation that RTT is rarely unchanged. On CPU 0, repeated release runs put the existing pristine `record()` path around **6.3-6.5 ns/ACK** and `record + window refresh` around **10.2-10.8 ns/ACK**. The exact difference is not additive because the loops share timestamp/RTT setup, but it establishes that the congestion-window refresh is a material fraction of this controller-only model rather than a sub-percent arithmetic detail. The original in-process `window-cached-tx` comparison was too order-sensitive to justify production by itself, so a later dedicated independent-process benchmark re-tested the exact conversion in the current loss-active reciprocal path. `BRUTAL_TX_CAST_BENCH_MODE=baseline|cached` produced **11.536 vs 5.440 ns/ACK** medians across 12 alternating CPU-0-pinned pairs, with a **0.474x** paired ratio median and **12/12** favorable pairs. Production therefore caches the exact `tx_bps as f64` conversion once at Brutal activation while retaining the original `u64` for pacing/debug semantics; the multiplication order and integer cwnd result remain unchanged. See `TCP_BRUTAL_TX_CAST_CACHE.md`.
...
`BRUTAL_PRISTINE_STORE_BENCH_MODE=baseline|skip` revisits the no-loss ACK-rate store against the **current** split-ACK/rollover-only production shape rather than the older generic record loop. Both variants retain the same pristine/debug condition and moving ~80 ms RTT/window arithmetic; the candidate only avoids rewriting `reciprocal_ack_rate = 1.0` after a legal cached-second pristine state has already established that exact value. Twelve alternating CPU-0-pinned independent-process pairs at 100 million ACK events each measured **3.777 ns/ACK** baseline versus **3.655 ns/ACK** candidate medians, a **0.965x** paired ratio median with **11/12** favorable pairs. One candidate sample was a clear scheduling/frequency outlier at 4.926 ns/ACK; the other paired ratios ranged **0.944x-0.985x**. Production therefore skips the redundant pristine store while loss-active and debug ACKs still execute the existing rate update path. See `TCP_BRUTAL_PRISTINE_ACK_STORE.md`; this is a controller-only microbenchmark, not a server-throughput claim.

`BRUTAL_SENT_DISPATCH_BENCH_MODE=baseline|active-fast` tests the even hotter per-packet `Controller::on_sent()` dispatch after Brutal activation. The candidate returns immediately after the required `brutal_active` test instead of also checking the BBR option that activation has already cleared. With 200 million callbacks/process, three warmup pairs, and twelve alternating CPU-0-pinned measured pairs, the medians were **1.377 vs 1.367 ns/sent** and the paired ratio median was **0.981x**. Only **8/12** pairs favored the candidate, with ratios ranging **0.823x-1.184x** and clear frequency-related bimodality. That is below the reliable signal floor, so production `on_sent()` remains unchanged. See `TCP_BRUTAL_SENT_DISPATCH.md`.
