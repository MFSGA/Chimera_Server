...
#[test]
fn handler_alter_inbound_socks_returns_not_user_manager() {
    trace_step(
        "==== test handler_alter_inbound_socks_returns_not_user_manager start ====",
    );
    let harness = Harness::start().expect("failed to start test harness");
    let add_operation = AddUserOperation {
        user: Some(User {
            level: 0,
            email: "grpc-e2e-added-user".to_string(),
            account: None,
        }),
    };
...
#[test]
fn handler_get_inbound_users_returns_not_user_manager_for_socks() {
    trace_step(
        "==== test handler_get_inbound_users_returns_not_user_manager_for_socks start ====",
    );
    let harness = Harness::start().expect("failed to start test harness");
...
#[test]
fn handler_get_inbound_users_count_returns_not_user_manager_for_socks() {
    trace_step(
        "==== test handler_get_inbound_users_count_returns_not_user_manager_for_socks start ====",
    );
    let harness = Harness::start().expect("failed to start test harness");
