...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
#[ignore = "starts Chimera as SOCKS server and ./xray as a plain VLESS upstream"]
async fn chimera_vless_plain_outbound_interoperates_with_xray_inbound() {
    let workspace = workspace_root();
    let work_dir = create_test_dir("vless-plain-outbound");
    let echo_addr = start_tcp_echo_server();
    let xray_vless_port = free_localhost_port();
    let chimera_socks_port = free_localhost_port();
    let xray_config_path = work_dir.join("xray-vless-inbound.json");
    let chimera_config_path = work_dir.join("chimera-vless-outbound.json");
    ...
    assert_socks5_echo(
        socks_addr,
        echo_addr,
        b"plain VLESS outbound through Xray inbound",
    );
    assert_socks5_echo(socks_addr, echo_addr, &deterministic_payload(64 * 1024));
    assert_socks5_domain_echo(
        socks_addr,
        "localhost",
        echo_addr.port(),
        b"plain VLESS outbound domain target",
    );
}
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
#[ignore = "starts Chimera SOCKS UDP and ./xray as a Shadowsocks UDP upstream"]
async fn chimera_shadowsocks_udp_outbound_interoperates_with_xray_inbound() {
    let workspace = workspace_root();
    let work_dir = create_test_dir("shadowsocks-udp-outbound");
    let udp_echo_addr = start_udp_echo_server().await;
    let xray_shadowsocks_port = free_localhost_port();
    let chimera_socks_port = free_localhost_port();
    let xray_config_path = work_dir.join("xray-shadowsocks-udp-inbound.json");
    let chimera_config_path = work_dir.join("chimera-shadowsocks-udp-outbound.json");
    let password = "chimera-shadowsocks-udp-password";
        ...
        json!({
            ...
            "routing": {
                "rules": [{
                    ...
                    "inboundTag": ["socks-in"],
                    "network": "udp",
                    "outboundTag": "to-xray-shadowsocks-udp"
                }]
            }
        }),
    );
    ...
    assert_socks5_udp_echo(
        socks_addr,
        udp_echo_addr,
        b"Shadowsocks UDP outbound through Xray inbound",
    )
    .await;
...
async fn chimera_socks_no_auth_outbound_interoperates_with_xray_inbound() {
    ...
    let chimera_config_path = work_dir.join("chimera-socks-outbound.json");
    ...
    assert_socks5_echo(
        socks_addr,
        echo_addr,
        b"no-auth SOCKS outbound through Xray inbound",
    );
    assert_socks5_echo(socks_addr, echo_addr, &deterministic_payload(64 * 1024));
    assert_socks5_domain_echo(
        socks_addr,
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
#[ignore = "starts Chimera as SOCKS server and ./xray as a TLS password SOCKS upstream"]
async fn chimera_socks_password_tls_outbound_interoperates_with_xray_inbound() {
    let workspace = workspace_root();
    ...
    let chimera_config_path = work_dir.join("chimera-socks-tls-outbound.json");
    let certificate_path = work_dir.join("localhost-cert.pem");
    let private_key_path = work_dir.join("localhost-key.pem");
    let generated =
        rcgen::generate_simple_self_signed(["localhost".to_string()]).unwrap();
    ...
    assert_socks5_echo(
        socks_addr,
        echo_addr,
        b"TLS password SOCKS outbound through Xray inbound",
    );
    assert_socks5_echo(socks_addr, echo_addr, &deterministic_payload(64 * 1024));
    assert_socks5_domain_echo(
        socks_addr,
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
#[ignore = "starts Chimera as SOCKS server and ./xray as a plain Trojan upstream"]
async fn chimera_trojan_plain_outbound_interoperates_with_xray_inbound() {
    let workspace = workspace_root();
    ...
    let chimera_config_path = work_dir.join("chimera-trojan-outbound.json");
    let password = "chimera-trojan-password";
    ...
    let mut xray = start_xray(&workspace, &work_dir, &xray_config_path);
    wait_for_tcp(SocketAddr::from((Ipv4Addr::LOCALHOST, xray_trojan_port)));
    xray.assert_running();
    ...
    assert_socks5_echo(
        socks_addr,
        echo_addr,
        b"plain Trojan outbound through Xray inbound",
    );
    assert_socks5_echo(socks_addr, echo_addr, &deterministic_payload(64 * 1024));
    assert_socks5_domain_echo(
        socks_addr,
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
#[ignore = "starts Chimera as SOCKS server and ./xray as a TLS Trojan upstream"]
async fn chimera_trojan_tls_outbound_interoperates_with_xray_inbound() {
    let workspace = workspace_root();
    ...
    let chimera_config_path = work_dir.join("chimera-trojan-tls-outbound.json");
    let certificate_path = work_dir.join("localhost-cert.pem");
    let private_key_path = work_dir.join("localhost-key.pem");
    let generated =
        rcgen::generate_simple_self_signed(["localhost".to_string()]).unwrap();
    ...
    fs::write(&certificate_path, &certificate_pem).unwrap();
    fs::write(&private_key_path, generated.signing_key.serialize_pem()).unwrap();
    let password = "chimera-trojan-tls-password";
        ...
        json!({
            ...
            "routing": {
                "rules": [{
                    "type": "field",
                    "inboundTag": ["socks-in"],
                    "outboundTag": "to-xray-trojan-tls"
                }]
            }
        }),
    );
    ...
    assert_socks5_echo(
        socks_addr,
        echo_addr,
        b"TLS Trojan outbound through Xray inbound",
    );
    assert_socks5_echo(socks_addr, echo_addr, &deterministic_payload(64 * 1024));
    assert_socks5_domain_echo(
        socks_addr,
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
#[ignore = "starts Chimera as SOCKS server and ./xray as a REALITY VLESS upstream"]
async fn chimera_vless_reality_outbound_interoperates_with_xray_inbound() {
    let workspace = workspace_root();
    let work_dir = create_test_dir("vless-reality-outbound");
    let echo_addr = start_tcp_echo_server();
    let reality_dest_addr = start_tls13_dest(&workspace).await;
    let xray_vless_port = free_localhost_port();
    let chimera_socks_port = free_localhost_port();
    let xray_config_path = work_dir.join("xray-vless-reality-inbound.json");
    let chimera_config_path = work_dir.join("chimera-vless-reality-outbound.json");
    ...
    assert_socks5_echo(
        socks_addr,
        echo_addr,
        b"REALITY VLESS outbound through Xray inbound",
    );
    assert_socks5_echo(socks_addr, echo_addr, &deterministic_payload(64 * 1024));
    assert_socks5_domain_echo(
        socks_addr,
        "localhost",
        echo_addr.port(),
        b"REALITY VLESS outbound domain target",
    );
}

#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
#[ignore = "starts Chimera as SOCKS server and ./xray as a WebSocket VLESS upstream"]
async fn chimera_vless_websocket_outbound_interoperates_with_xray_inbound() {
    let workspace = workspace_root();
    let work_dir = create_test_dir("vless-websocket-outbound");
    let echo_addr = start_tcp_echo_server();
    let xray_vless_port = free_localhost_port();
    let chimera_socks_port = free_localhost_port();
    let xray_config_path = work_dir.join("xray-vless-websocket-inbound.json");
    let chimera_config_path = work_dir.join("chimera-vless-websocket-outbound.json");
            ...
            "outbounds": [{
                ...
                "streamSettings": {
                    ...
                    "wsSettings": {
                        "host": "ws.example.test",
                        "path": "/chimera-ws",
                        "headers": {"X-Chimera-Test": "websocket"}
                    }
                }
            }],
            "routing": {
    ...
    assert_socks5_echo(
        socks_addr,
        echo_addr,
        b"WebSocket VLESS outbound through Xray inbound",
    );
    assert_socks5_echo(socks_addr, echo_addr, &deterministic_payload(64 * 1024));
    assert_socks5_domain_echo(
        socks_addr,
        "localhost",
        echo_addr.port(),
        b"WebSocket VLESS outbound domain target",
    );
}

#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
#[ignore = "starts Chimera as SOCKS server and ./xray as a TLS WebSocket VLESS upstream"]
async fn chimera_vless_tls_websocket_outbound_interoperates_with_xray_inbound() {
    let workspace = workspace_root();
    let work_dir = create_test_dir("vless-tls-websocket-outbound");
    let echo_addr = start_tcp_echo_server();
    let xray_vless_port = free_localhost_port();
    let chimera_socks_port = free_localhost_port();
    let xray_config_path = work_dir.join("xray-vless-tls-websocket-inbound.json");
    let chimera_config_path =
        work_dir.join("chimera-vless-tls-websocket-outbound.json");
    let generated = rcgen::generate_simple_self_signed(["localhost".to_string()])
        .expect("generate WebSocket TLS certificate");
        ...
        json!({
            ...
            "routing": {
                "rules": [{
                    "type": "field",
                    "inboundTag": ["socks-in"],
                    "outboundTag": "to-xray-tls-websocket"
                }]
            }
        }),
    );
    ...
    assert_socks5_echo(
        socks_addr,
        echo_addr,
        b"TLS WebSocket VLESS outbound through Xray inbound",
    );
    assert_socks5_echo(socks_addr, echo_addr, &deterministic_payload(64 * 1024));
    assert_socks5_domain_echo(
        socks_addr,
        "localhost",
        echo_addr.port(),
        b"TLS WebSocket VLESS outbound domain target",
    );
}
