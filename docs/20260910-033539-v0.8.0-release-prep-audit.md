...
Hysteria2 is not a viable release goal on the current candidate. Its positive Xray TCP+UDP interoperability test was executed three times and failed all three times with `SOCKS UDP echo timeout: Elapsed(())`. Three narrower Hysteria2 TCP/auth tests passed in the same environment, so the known blocker is specifically the real Xray-client UDP path rather than all Hysteria2 connectivity. Because current `master` includes Hysteria2 behavior changes, this reproducible real-client failure is also a stability concern for releasing current HEAD even if Shadowsocks is named as the primary goal.
...
4. Re-run the four recorded Shadowsocks real-client interoperability commands on the exact version-bumped release candidate. Resolve or explicitly exclude the current Hysteria2 UDP regression before shipping any current-HEAD stable release.
...
- Known real-client regression outside the selected goal but present in current HEAD: **Hysteria2 UDP, 3/3 positive interop retries FAIL; TCP/auth checks PASS**.
- Stable release recommendation: **NO-GO from current HEAD until the multi-goal scope problem and Hysteria2 UDP regression are resolved or a clean single-goal release source is prepared**.
