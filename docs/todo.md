...
当前结论：入站、路由、Stats/Handler/Routing/Observatory 控制面已经较完整。出站运行时已从仅支持 `freedom`/`blackhole` 推进到支持 VLESS TCP，并完成可复用的 TCP/TLS 静态 transport pipeline；REALITY、WebSocket、HTTP Upgrade、gRPC、其他代理出站、通用 sniffing、Xray DNS/FakeDNS、mKCP、TUN、Reverse、WireGuard 和部分 XHTTP client/xmux/download 语义仍未完成。
...
- [ ] 支持通用 outbound transport pipeline：TCP 与静态 TLS 已完成，支持系统根或内联 `usage=verify` CA、SNI、ALPN、TLS 1.2/1.3 边界和 session resumption 开关，并通过 Xray 26.2.6 TLS 1.3 + ALPN 真实互通；动态 sender settings、REALITY、WebSocket、HTTPUpgrade 和 gRPC 仍待完成。
