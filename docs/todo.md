...
- [x] 完成 VLESS typed outbound 主路径：静态 `settings/streamSettings` 与动态 Xray `SenderConfig` 均可表达 TCP、TLS、REALITY、WebSocket、HTTP Upgrade 和 gRPC，并由同一 registry 编译器校验。`via`、proxy chaining、Mux、socket options、mask、QUIC 参数和非 AsIs target strategy 继续 fail-closed。
...
- [x] 完成 VLESS outbound transport pipeline：TCP、TLS、REALITY、WebSocket、HTTP Upgrade 和 gRPC 均已接入统一“安全层 + 应用传输层”模型，并支持静态 JSON 与动态 HandlerService SenderConfig。gRPC 支持单流 `Tun`、默认 `GunService`、authority、userAgent 与 serviceName，并可叠加 TLS 或 REALITY；multiMode、keepalive 调优和自定义 method path 继续 fail-closed。plain/TLS/REALITY gRPC 均已通过 Xray 26.2.6 真实互通，覆盖 IP、域名和 64KiB。
- [x] 支持 Trojan TCP/UDP outbound：标准 `servers` 与简化 `address/port/password` 配置、动态 `xray.proxy.trojan.ClientConfig`、ListOutbounds 回显、SHA-224 hex 认证头、原始 domain/IP target，并复用完整 transport pipeline。UDP 使用 command `0x03` 和 `address + u16 length + CRLF + payload` 分帧，每包建立有界 one-shot stream association，覆盖固定目标、目标化/session/XUDP、SOCKS5、Dokodemo、Shadowsocks inbound、Hysteria2 与 TUIC。plain/TLS 均已通过 Xray 26.2.6 TCP/UDP 真实互通，UDP 覆盖文本与 1200-byte datagram；持久 association 尚未实现。
- [x] 支持 SOCKS5 TCP/UDP outbound：单服务器、NO_AUTH 与 RFC1929 用户名密码、标准 `servers/users` 与简化配置、动态 `xray.proxy.socks.ClientConfig`、ListOutbounds 回显、CONNECT/UDP ASSOCIATE 响应严格校验和原始 domain/IP target。UDP 使用有界 one-shot association，保持控制流存活，严格要求 RSV=0/FRAG=0，并覆盖固定目标、目标化/session/XUDP、SOCKS5、Dokodemo、Shadowsocks inbound、Hysteria2 与 TUIC。plain no-auth 与 TLS+password 均已通过 Xray 26.2.6 TCP/UDP 真实互通，UDP 覆盖文本与 1200-byte datagram；持久 association 与 fragmentation 尚未实现。
