...
mod tests {
    ...
    #[test]
    fn socket_tproxy_wraps_dedicated_grpc_listener() {
        let inbound: InboudItem = serde_json::from_value(serde_json::json!({
            "listen": "127.0.0.1",
            "port": 443,
            "protocol": "vless",
        ...
        let ServerProxyConfig::TransparentSocket { inner } = config.protocol else {
            panic!("sockopt.tproxy must wrap the gRPC listener");
        };
    ...
    #[test]
    fn trusted_forwarded_headers_wrap_websocket_listener() {
        let inbound: InboudItem = serde_json::from_value(serde_json::json!({
            "listen": "127.0.0.1",
            "port": 443,
            "protocol": "vless",
            "tag": "vless-ws-forwarded",
            "settings": {
                "clients": [{"id": "3ac9b383-75a1-431c-8184-106c80eb2273"}],
                "decryption": "none"
            },
        ...
        let ServerProxyConfig::TrustedForwardedHeaders { names, inner } =
            ...
            panic!("trustedXForwardedFor must wrap the WebSocket listener");
        };
        assert_eq!(names, vec!["X-Trusted-Proxy"]);
        assert!(matches!(*inner, ServerProxyConfig::Websocket { .. }));
    }

    #[cfg(all(feature = "vless", feature = "grpc_transport"))]
    #[test]
    fn trusted_forwarded_headers_wrap_grpc_listener() {
        let inbound: InboudItem = serde_json::from_value(serde_json::json!({
            "listen": "127.0.0.1",
            "port": 443,
            "protocol": "vless",
            "tag": "vless-grpc-forwarded",
            "settings": {
                "clients": [{"id": "3ac9b383-75a1-431c-8184-106c80eb2273"}],
                "decryption": "none"
            },
        ...
        let ServerProxyConfig::TrustedForwardedHeaders { names, inner } =
            ...
            panic!("trustedXForwardedFor must wrap the gRPC listener");
        };
        assert_eq!(names, vec!["X-Trusted-Proxy"]);
        assert!(matches!(*inner, ServerProxyConfig::Grpc(_)));
    }
    ...
    #[cfg(all(feature = "vless", feature = "grpc_transport", target_os = "linux"))]
    #[test]
    fn socket_tcp_mptcp_wraps_dedicated_grpc_listener() {
        let inbound: InboudItem = serde_json::from_value(serde_json::json!({
            "listen": "127.0.0.1",
            "port": 443,
            "protocol": "vless",
        ...
        let config = ServerConfig::try_from(inbound).unwrap();
        let ServerProxyConfig::TcpMultipath { inner } = config.protocol else {
            panic!("tcpMptcp must wrap the gRPC listener");
        };
        assert!(matches!(*inner, ServerProxyConfig::Grpc(_)));
    }
    ...
    fn socket_tcp_mptcp_wraps_xhttp_listener() {
        ...
        let config = ServerConfig::try_from(inbound).unwrap();
        let ServerProxyConfig::TcpMultipath { inner } = config.protocol else {
            panic!("tcpMptcp must wrap the XHTTP listener");
        };
        assert!(matches!(*inner, ServerProxyConfig::Xhttp { .. }));
    }
    ...
    #[cfg(all(feature = "vless", feature = "tls", target_os = "linux"))]
    #[test]
    fn socket_tcp_mptcp_rejects_xhttp_http3() {
        let inbound: InboudItem = serde_json::from_value(serde_json::json!({
            "listen": "::1",
            "port": 443,
            "protocol": "vless",
            "tag": "vless-xhttp-h3-mptcp",
            "settings": {
                "clients": [{"id": "3ac9b383-75a1-431c-8184-106c80eb2273"}],
                "decryption": "none"
            },
