...
fn test_handshake_multiple_calls_accumulate() {
    ...
    encrypt_handshake_to_records(&batch1, &key, &iv, &mut seq, &mut ciphertext_buf)
        .unwrap();
    ...
    encrypt_handshake_to_records(&batch2, &key, &iv, &mut seq, &mut ciphertext_buf)
        .unwrap();
...
fn test_plaintext_and_handshake_produce_different_inner_content_type() {
    ...
    encrypt_plaintext_to_records(&mut app_data, &key, &iv, &mut seq1, &mut app_buf)
        .unwrap();
