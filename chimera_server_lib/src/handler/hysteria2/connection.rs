...
mod tests {
    ...
    fn auth_request_at(auth: &str, uri: &str) -> Request<()> {
        Request::builder()
    ...
    fn transport_auth_disables_udp_and_users_take_precedence() {
        ...
        assert!(
            validate_auth_request(
                auth_request_at("fallback-auth", "https://example.com/auth"),
                &config,
            )
            .is_err()
        );
        assert!(
            validate_auth_request(
                auth_request_at("fallback-auth", "https://hysteria/auth?extra=1"),
                &config,
            )
            .is_err()
        );
