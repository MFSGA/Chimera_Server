...
| Area | Status |
| --- | --- |
| mKCP transport | Xray `kcp` / `mkcp` aliases and current `kcpSettings` defaults/validation materialize as an explicit `Mkcp` server transport capability. A prepared UDP listener, Xray-compatible raw KCP segment codec, and `(remote, conv)` demux scaffold now exist, but startup still fails closed with `Unsupported` until connection/ARQ handling can deliver a real stream; interoperability is not yet claimed. |
