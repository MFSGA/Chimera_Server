...
| VLESS Reverse | **Portal-first TCP: Partial / verified with Xray.** Current Reverse account metadata (`reverse = 7`), command `0x04`, Xray Mux/control wire, bounded TCP worker/session core, ACTIVE/DRAIN control lifecycle, dynamic `reverse.tag` routing capability, Portal registry, and DokodemoDoor fixed-port routing are implemented. Fixed Xray-core `v26.9.9` Bridge interoperability passes for RAW VLESS/TCP and VLESS/TCP+TLS loopback echo. This does **not** claim full Reverse support: Chimera Bridge, UDP/XUDP, sniffing, dynamic mappings, and broader transport/security combinations remain pending. See [`VLESS_REVERSE_DESIGN.md`](../../VLESS_REVERSE_DESIGN.md). |
