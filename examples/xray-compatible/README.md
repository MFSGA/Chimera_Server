...
| VLESS Reverse | **Account/command boundary only; runtime still unavailable.** The `vless-reverse` feature recognizes and preserves current inbound/outbound Reverse account metadata including protobuf `reverse = 7`; command `0x04` is parsed as Xray's addressless Reverse request and ordinary/Reverse user authorization is fail-closed. Authenticated `0x04` currently stops at an explicit Portal-runtime-unavailable boundary. Xray Mux, Reverse control sessions, worker selection, fixed-port exposure, and interoperability are not implemented or claimed yet. See [`VLESS_REVERSE_DESIGN.md`](../../VLESS_REVERSE_DESIGN.md). |
