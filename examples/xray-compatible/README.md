...
| VLESS Reverse | **TCP RAW/TLS + UDP RAW roles: Partial / verified bidirectionally with Xray.** Reverse account metadata (`reverse = 7`), command `0x04`, Xray Mux/control wire, Portal registry/routing, bounded client/server Mux workers, ACTIVE/DRAIN control, DokodemoDoor fixed-port routing, and ordinary Reverse UDP packet sessions are implemented. Fixed Xray-core `v26.9.9` interoperability passes in both directions for RAW VLESS/TCP, VLESS/TCP+TLS, and RAW Reverse UDP: Xray Bridge -> Chimera Portal and Chimera Bridge -> Xray Portal. Chimera Bridge -> Xray Portal is also Xray-verified over WebSocket without early data. Bridge outbound `reverse.sniffing` now uses Xray's native SniffingConfig wire and is fixed-Xray-verified over RAW for HTTP Host, TLS SNI, routeOnly, domainsExcluded and ipsExcluded. Reverse Bridge outbound email/level now populate the virtual inbound routing/traffic context and the VLESS UUID is retained as the user-domain policy identity; fixed-Xray RAW interoperability verifies `user` routing and verifies Xray Portal source propagation into Chimera Freedom `proxyProtocol: 1`, while policy identity/traffic accounting have local regression coverage. Freedom `proxyProtocol` 0/1/2 is preserved through static/gRPC config and v1/v2 TCP header emission. `metadataOnly` and FakeDNS remain fail-closed. In the fixed baseline, Reverse Mux source/local metadata and ordinary Mux XUDP GlobalID are mutually exclusive on NEW frames, so Chimera does not invent a Reverse GlobalID reattachment wire. VLESS Reverse AddUser/RemoveUser hot updates, first-RVS lazy virtual-outbound publication, and route removal are implemented; fixed-Xray HandlerService network interoperability now verifies AddUser(reverse) -> command `0x04` -> lazy route publication -> RemoveUser, while shared-tag and normal-outbound conflict edges have local regression coverage, including Xray's destructive RemoveUser behavior when a reverse tag collides with a normal outbound. WebSocket early data, HTTPUpgrade/gRPC/REALITY and broader transport/security combinations also remain pending. See [`VLESS_REVERSE_DESIGN.md`](../../VLESS_REVERSE_DESIGN.md). |
