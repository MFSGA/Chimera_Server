...
| VLESS Reverse | **Standalone Mux TCP core implemented; Portal integration still unavailable.** In addition to current Reverse account metadata, protobuf `reverse = 7`, command `0x04`, and the Xray-aligned Mux/control wire codec, Chimera now has a bounded Mux TCP client worker, session manager, least-loaded ACTIVE-only picker, `udp://reverse:0` control lifecycle, Xray END close semantics, orphan-session handling, and physical-close/backpressure tests. Authenticated `0x04` still stops before registering that physical connection into a Reverse registry, so DokodemoDoor/routing cannot use these workers yet. Fixed-port exposure and Xray interoperability are not claimed until Batch E. See [`VLESS_REVERSE_DESIGN.md`](../../VLESS_REVERSE_DESIGN.md). |
