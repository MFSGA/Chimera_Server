...
| VLESS Reverse | **TCP RAW/TLS roles: Partial / verified bidirectionally with Xray.** Reverse account metadata (`reverse = 7`), command `0x04`, Xray Mux/control wire, Portal registry/routing, bounded client/server Mux workers, ACTIVE/DRAIN control, and DokodemoDoor fixed-port routing are implemented. Fixed Xray-core `v26.9.9` interoperability passes in both directions for RAW VLESS/TCP and VLESS/TCP+TLS: Xray Bridge -> Chimera Portal and Chimera Bridge -> Xray Portal. Chimera Bridge uses the Xray-aligned 2-second supervised monitor, retries failed initial dials, scales when average active connections exceed 16, removes failed workers, and reconnects after Portal restart. Chimera Bridge -> Xray Portal is also Xray-verified over WebSocket without early data. This does **not** claim full Reverse support: UDP/XUDP, sniffing, dynamic management/mappings, WebSocket early data, HTTPUpgrade/gRPC/REALITY and broader transport/security combinations remain pending. See [`VLESS_REVERSE_DESIGN.md`](../../VLESS_REVERSE_DESIGN.md). |
