...
| VLESS Reverse | **Wire boundary only; runtime still unavailable.** The `vless-reverse` feature preserves current Reverse account metadata including protobuf `reverse = 7`, parses Xray's addressless command `0x04`, and now has an Xray-aligned Reverse Mux frame/control wire codec with TCP/UDP target metadata, Reverse source/local fields, ACTIVE/DRAIN control, transfer-type mapping, fixed vectors, and fail-closed length/status/session-ID validation. Authenticated `0x04` still stops before any Mux session/worker runtime. Worker selection, control-session lifecycle, fixed-port exposure, and interoperability are not implemented or claimed yet. See [`VLESS_REVERSE_DESIGN.md`](../../VLESS_REVERSE_DESIGN.md). |
