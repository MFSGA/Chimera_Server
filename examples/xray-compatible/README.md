...
| VLESS Reverse | **Config-only / fail-closed.** The `vless-reverse` feature now recognizes and preserves inbound `clients[].reverse.tag` from file config and gRPC/UserManager account payloads; without the feature it returns an explicit capability error, Reverse-only users cannot fall back to ordinary forward proxying, simplified outbound `reverse` is recognized but rejected until the Chimera Bridge role exists, and non-null legacy root `reverse` is explicitly rejected. Command `0x04`, Xray Mux, Portal runtime, fixed-port exposure, and interoperability are not implemented or claimed yet. See [`VLESS_REVERSE_DESIGN.md`](../../VLESS_REVERSE_DESIGN.md). |
